    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {{-- Join by Token --}}
                <div class="bg-white rounded-2xl border border-[#e2e8f0] p-6 shadow-sm col-span-1 lg:col-span-2">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-extrabold text-[#1b364b]">Got an Invitation Token?</h3>
                    </div>
                    <form action="{{ route('invitation.join.post') }}" method="POST" class="flex gap-3 max-w-xl">
                        @csrf
                        <input type="text" name="token" placeholder="Paste your token here..."
                            class="flex-1 rounded-lg border-[#dae2ec] text-sm focus:ring-[#2563eb] focus:border-[#2563eb]"
                            required>
                        <button type="submit"
                            class="px-6 py-2 bg-[#2563eb] text-white font-bold rounded-lg text-sm hover:bg-[#1a4ac4] transition-all">Join
                            Collocation</button>
                    </form>
                </div>
